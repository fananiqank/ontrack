...
foreach($database->query("select a.id,case when typereq=1 then 'Permintaan Akun' when typereq=2 then 'Izin Akses Data' when typereq=4 then 'Penutupan Akun' end typereqshow,c.name as namepeople,d.name as namedept,e.name as locationname,a.title,e.peopleid,a.locationid,a.norequest
 from people_dtl_request a join people_dtl_approve b on a.id =b.peopledtlid 
join people c on a.peopleid=c.id
left join clients d on a.clientid=d.id
left join locations e on a.locationid=e.id
where (itstatus is null and CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), b.headappdate),':',1),0) AS SIGNED) > '24')") as $tclose){
        foreach($database->query("select email,name from people where approval_type = '2'") as $email){
            include "./phpmailer/pushemailreminderitapprove.php";    
        }
