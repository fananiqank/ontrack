...
if ($route == "pdf") {
	...
	if($type == "assetlabel") {
		...
		if(getConfigValue("manual_qrvalue") == 'true') {
			...
			$qrvalue = "http://192.168.31.25/eraticket/index.php?ids=" .$asset['id'];
		}
		...
		<table>
			<tr>
				...
				<td align="left" style="font-size:10px;"><b>'.$asset['nodenumber'].'</b></td>
				<td align="right" style="font-size:6px;"><b>'.$typeaset.'</b></td>
			</tr>
			<tr><td valign="top" style="font-size:9px;"><b>'.$qrtext.'</b></td></tr>
			<tr><td valign="top" style="font-size:6px;"><b>'.$typepurch.'</b></td></tr>
			<tr><td valign="top" style="font-size:7px;"><b>'.$asset[ipaddress].'</b></td></tr>
		</table>
			</div>';
...
// GENERAL
if($isAdmin) {
	$arTicketsCount = $database->count("tickets", ["status" => ["Open"]] );
...
if ($route == "inventory/assets/manage") {
	...
	$getdataeis = file_get_contents("http://192.168.51.26:5000/restfulapiocs/public/ocshw/".$nameasetocs);
...
if ($route == "inventory/licenses/manage") {
	...
	//$license = getRowById("licenses",$_GET['id']);
	$licenses = $database->query("select a.id,a.name,a.tag,a.categoryid,sum(b.seats) as seats from  licenses a left join licenses_dtl b on a.id=b.licensesid and b.statusid > 0 where a.statusid > 0 and a.id = '$_GET[id]' 
...
if ($route == "tickets/ar") {
	...
	if($liu[roleid] == 4 || $liu[roleid] == 5) {
		...
		$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.status in ('Open') and a.typeticket in (3) order by a.id DESC");
	} 
	else {
...
if ($route == "tickets/active") {
	...
	if($isAdmin) {
		...
		if($liu[roleid] == 4 || $liu[roleid] == 5) {
			...
			$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.status not in ('Closed','Reject','Finished') and a.typeticket in (3,4) order by a.id DESC");
		} else if($liu[roleid] == 3 || $liu[roleid] == 6) {
			$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.status not in ('Closed','Reject','Finished') and a.typeticket in (3) order by a.id DESC");
		} else {
			$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.status not in ('Closed','Reject','Finished') order by a.id DESC");
...
if ($route == "tickets/all") {
	...
	if($isAdmin) {
		...
		if($liu[roleid] == 4 || $liu[roleid] == 5) {
			...
			$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.typeticket in (3,4) order by a.id DESC"); 
			//$tickets = $database->select("tickets", "*", ["typeticket" => [3,4], "ORDER" => ["id" => "DESC"]]);
		} else if($liu[roleid] == 3 || $liu[roleid] == 6) {
			$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.typeticket in (3) order by a.id DESC"); 
		...
		$tickets = $database->query("select a.*,c.name as picuser,f.name as picapprove,d.name as picadmin, e.name as picfinish, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.timestamp),':',1),0) AS SIGNED) as sla, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(now(), a.approvedate),':',1),0) AS SIGNED) as slamod, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.approvedate, a.timestamp),':',1),0) AS SIGNED) as slaapprove, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.finishdate, a.approvedate),':',1),0) AS SIGNED) as slafinished, CAST(COALESCE(SUBSTRING_INDEX(TIMEDIFF(a.closedate, a.finishdate),':',1),0) AS SIGNED) as slaclosed,g.name as slaname,g.sla as slatime from tickets a left join clients b on a.clientid=b.id left join people c on a.userid=c.id left join people f on a.approvedby=f.id left join people d on a.adminid=d.id left join people e on a.finishby=e.id left join tickets_sla g on a.slaid=g.id where a.clientid = $liu[clientid] order by a.id DESC"); 
...
if ($route == "reports/view") {
	...
	elseif($_GET['report'] == "summarylicense") {
		...
		//$assets = $database->query("select * from assets where statusid <> 0 ".$typeasset.$clientid.$categoryid.$statusid."");
		$licenses = $database->query("select a.id,a.name,a.tag,a.categoryid,sum(b.seats) as seats from  licenses a left join licenses_dtl b on a.id=b.licensesid and b.statusid > 0 where a.statusid > 0 ".$categoryid.$statusid.$namlic." GROUP BY a.id,a.name,a.tag,a.categoryid");
		$asetlicense = $database->query("select * from (select a.name as licensename,c.`name` as assetname,c.tag,c.machine_id,b.notes,
case when b.licenseid = b.inslicenseid then 'V' else '-' end downgrade from licenses a join licenses_assets b on a.id=b.licenseid join assets c on b.assetid=c.id where a.statusid <> 0 and b.typeasset = 1 ".$categoryid.$statusid.$namlic." union select a.name as licensename,c.`name` as assetname,c.tag,'VM' as machine_id,b.notes,
case when b.licenseid = b.inslicenseid then 'V' else '-' end downgrade from licenses a join licenses_assets b on a.id=b.licenseid join virtuals c on b.assetid=c.id where a.statusid <> 0 and b.typeasset = 2 ".$categoryid.$statusid.$namlic.") b order by licensename");
