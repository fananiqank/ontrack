...
function signIn($email,$password) //login and set session
	{
		...
		if ($internal == "1") {
			...
			header("Location:?route=dashboard&th=".date("Y"));
			//exit;
		}
		elseif ($ldap == 1) {
...
function isAuthorized($action) {
	...
	if(!in_array($action,$perms)) { setStatus("1"); header("Location:?route=dashboard&th=".date("Y")); exit; }
}
...
function sendEmaildua($to,$subject,$message,$clientid="0",$peopleid="0",$ccs=array(),$attachments=array()) //send email
	{	
		
		global $database;
		global $scriptpath;
		include "./phpmailer/pushemail2.php"; 
		// $mail = new PHPMailer\PHPMailer\PHPMailer();
...
function counTicketsByDepartment($departmentid, $startdate, $enddate, $clientid=0, $typeticket) {
	global $database;
	$items = 0;
	if($typeticket > 0){
		if($typeticket == 1){$ttk=[1,2];}else if($typeticket == 2){$ttk=3;}
		if($clientid == "0") {
