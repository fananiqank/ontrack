...
$con=new Database();
			...
			if ($tclose['email'] != ''){
					...
					$mail->Subject = 'Canceled Request '.$tclose['typereqshow'].' by you';
					//Read an HTML message body from an external file, convert referenced images to embedded,
					
						$text="<p>Hello ".$tclose[namepeople].",<br><br>
								Your Requested has automatically canceled by the system, <br> 
								because it has exceeded the approval time limit.<br>
								Please create new request if you need.
        ...
        for($i=0;$i<$jmlkas;$i++){
            foreach($database->query("select email,name from people where id = '".$expakas[$i]."'") as $email){}
            	if ($email['email'] != ''){
					...
					//Set the subject line
					$mail->Subject = 'Canceled Request '.$tclose['typereqshow'];
					//Read an HTML message body from an external file, convert referenced images to embedded,
					
						$text="<p>Hello ".$email[name].",<br><br>
								Requested by $tclose[namepeople] has automatically canceled by the system, <br> 
								because it has exceeded the approval time limit.<br>
								Please create new request if you need.
