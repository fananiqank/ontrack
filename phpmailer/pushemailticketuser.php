...
$con=new Database();
		//departemen
		foreach($con->select("tickets a join people b on a.userid=b.id","a.*,b.name as namepeople,DATE(a.modifydate) as datenow","a.id = '$data[ticketid]'") as $dpt){}
        		...
        		if ($dpt['email'] != ''){
					...
					$mail->Subject = 'Ticket No. '.$dpt['ticket'].' New Reply : '.$data['status'];
					...
					if($data['status'] == 'Finished'){
						...
						if($dpt['people_dtl_request_id'] != ''){
							...
							$text .="You can track progress of this request regularly through <a href='http://192.168.31.25/eraticket/index.php?x=view&d=$data[ticketid]'>Link</a>. <br>
									Please do not remove the ticket number from the subject line.<br><br>Best regards,<br>IT Ticketing Admin<br></p>";
						}
					}
		...
		$expmention = explode(";",$dpt['mentionid']);
		$jumepps = count($expmention);
		$jumepp = $jumepps-1;
		for ($i=0; $i < $jumepp ; $i++) {
			foreach($database->query("select * from people where id = '".$expmention[$i]."'") as $mailmention){} 
			if ($mailmention['email'] != ''){
					//Create a new PHPMailer instance
					$mail = new PHPMailer;
					//Tell PHPMailer to use SMTP
					$mail->isSMTP();
					...
					$mail->addReplyTo('sby-helpdesk@eratex.co.id', 'IT Ticketing Eratex Djaja ,Tbk');
					//Set who the message is to be sent to
					$mail->addAddress($mailmention['email'] ,$mailmention['name']);
					//Set the subject line
					$mail->Subject = 'Ticket No. '.$dpt['ticket'].' New Reply : '.$data['status'];
					//Read an HTML message body from an external file, convert referenced images to embedded,
					
					$text="<p>Hello ".$mailmention['namepeople'].",<br><br>
								A new reply has been added to your ticket.<br>
								Ticket ID: ".$dpt['ticket']."<br>
								Subject  : ".$dpt['subject']."<br>
								".$messageReply."<br><br>
