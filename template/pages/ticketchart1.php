...
//select IN 
$selectaset = $database->query("select b.nodenumber,count(a.id) as jumasset 
from tickets a join assets b on a.assetid=b.id where a.assetid != '' and YEAR(`timestamp`) = '".$_GET['th']."'
GROUP BY a.assetid ORDER BY count(a.id) DESC limit 5");
$ninaset = 0;
...
<div class="col-lg-12 col-md-12">
  <button id="print-chart-btn1">Print Chart</button>
...
<script type="text/javascript">
  ...
  var barChartDatain = {
        labels: [
          <?php 
          $selectaset2 = $database->query("select b.nodenumber,count(a.id) as jumasset 
from tickets a join assets b on a.assetid=b.id where a.assetid != '' and YEAR(`timestamp`) = '".$_GET['th']."'
GROUP BY a.assetid ORDER BY count(a.id) DESC limit 5");
            foreach ($selectaset2 as $ssaset2) { 
                  echo "'".$ssaset2['nodenumber']."',";
              }
          ?>
        ],
        datasets: [{
    ...
    myChartin = new Chart(ctx, {
        ...
        options: {
          ...
          title: {
             display: true,
             text: 'Most 5 Assets Th.'+<?php echo $_GET['th']; ?>,
          },
          tooltips: {
            enabled: true,
            mode: 'index',
    ...
    $('#print-chart-btn1').on('click', function() {
      var canvas = document.querySelector("#myChartin");
