...
from tickets a join people b on a.finishby=b.id 
where a.finishby != '' and YEAR(`timestamp`) = '".$_GET['th']."'
GROUP BY a.finishby ORDER BY count(a.id) DESC limit 5");
$ninaset = 0;
...
<div class="col-lg-12 col-md-12">
  <button id="print-chart-btn4">Print Chart</button>
...
<script type="text/javascript">
  var ctx = document.getElementById("myChartst").getContext('2d');
  var barChartDatast = {
        labels: [
          <?php 
...
GROUP BY a.finishby ORDER BY count(a.id) DESC limit 5");
    ...
    myChartst = new Chart(ctx, {
        ...
        options: {
          ...
          title: {
             display: true,
             text: 'Most 5 IT Closed Ticket Th.'+<?php echo $_GET['th']; ?>,
          },
          tooltips: {
            enabled: true,
            mode: 'index',
    ...
    $('#print-chart-btn4').on('click', function() {
      var canvas = document.querySelector("#myChartst");
